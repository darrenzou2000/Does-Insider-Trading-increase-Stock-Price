...
# this class is the interface to easily access previously gathered data,
# The acutal scrapper is in scrapper.py and the data gathering from APIS is in DataGetter.py
class App():
    ...
    def printScrapped(self) -> None:
        ...
        for entry in self.scrapped["Scrapped"]:
            print(count, ":", entry["description"], "// Located at",
                  entry["filePath"], "// count", entry["count"])
            count += 1
        if (len(self.scrapped["Scrapped"]) == 0):
            print("(No data is found, please enter new)")
        self.setScrapper()

    # after a dataframe is selected or genereated, print out the normal data that people want to see like average,
    # percent that returns positive, and stocks that are still tradeable today.
    def generalData(self) -> None:
        result = Result(self.scrapper.getData())
        result.getPositive()
        result.getAVG()
        result.getActive()
    ...
    def setScrapper(self) -> None:
        ...
        if (action.isnumeric()):
            ...
            while (index >= len(self.scrapped["Scrapped"])):
                index = int(
                    input(
                        "out of range, please enter the number of the index you want to see: "
                    )) - 1
            ...
            if (action == "remove"):
                ...
                print(
                    "action not found, please enter one of the following: remove, new, redo"
                )
                self.setScrapper()
    ...
    def removefile(self):
        ...
        print(idx + 1, ":", self.scrapped["Scrapped"][idx]["description"],
              "// Located at", self.scrapped["Scrapped"][idx]["filePath"])
        confirm = input(
            "are you sure you want to delete the above? (y or n): ")
